...
from Models.SeEANet_N import SeEANet_N
...
def train_func(epochs=2000,modelName="SAT", train_num=5,
    ...
    windowLength=256, stepLength=1, delta_T=20, train_ratio=0.8, PreNum=5,
    formatted_time = '0', batch_size = 512, scheduler_gamma=0.5, motionL=[1, 2, 4]):
    ...
    emg_data, angle_data, afterClusterEmgIndex = make_datasets(data_Dir, peopleList, exp_class, cluster_num, fusionMethod, 
                                        windowLength, stepLength, delta_T, motionL)
    ...
    for train_index in range(train_num):
        ...
        elif modelName == "SAT_N":
            model = SATNet_N(PreNum, IndexL=afterClusterEmgIndex)
        elif modelName == "SeEANet_N":
            model = SeEANet_N(PreNum, IndexL=afterClusterEmgIndex)

        model = model.cuda()
        optimizer = optim.Adam(model.parameters(), lr=0.001)
        scheduler = StepLR(optimizer, step_size=100, gamma=scheduler_gamma)
    ...
    #     elif modelName == "SAT_E":
    #         model_eval = SATNet_E(PreNum, cluster_num=cluster_num)
    #     elif modelName == "SAT_N":
    #         model_eval = SATNet_N(PreNum, IndexL=afterClusterEmgIndex)
    #     elif modelName == "SeEANet_N":
    #         model_eval = SeEANet_N(PreNum, IndexL=afterClusterEmgIndex)
    ...
    #     json_path = os.path.join(checkpoint_save_Dir, f"metrics_{train_index+1}.json")
    #     # 将字典保存为 JSON 文件
...
def test_func(modelName="SAT", model_path = "nn",
    data_Dir="/home/admin123/SATData/data",
    peopleList=['S01'], exp_class="MJ",
    cluster_num=6, fusionMethod="PCA", batch_size=256, 
    windowLength=256, stepLength=1, delta_T=20, train_ratio=0.8, PreNum=3, motionL=[1,2,3,4,5]):
    ...
    # 解析数据集
    emg_data, angle_data, afterClusterEmgIndex = make_datasets(data_Dir, peopleList, exp_class, cluster_num, fusionMethod, 
                                        windowLength, stepLength, delta_T, motionL=motionL)
    ...
    # return (tru_angle_1, tru_angle_2, tru_angle_3, pre_angle_1, pre_angle_2, pre_angle_3)
    return (truthAngles, preAngles)
