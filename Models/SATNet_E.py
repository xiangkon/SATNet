...
class SATNet_E(nn.Module):
    def __init__(self, PreNum=3, PreLen=1, input_shape = [256, 6]):
        ...
        self.conv1 = nn.Conv1d(inceptionFilter*4, inceptionFilter, kernel_size=3, padding=1)
        self.relu1 = nn.ReLU()

        self.tranformer = TransformerPredictor(inceptionFilter, d_model=64, nhead=4, num_layers=1, dropout=0, pre_num=self.scale)

        self.gru1 = nn.GRU(input_size=self.scale, hidden_size=self.scale, num_layers=1, batch_first=True, bidirectional=True)
        self.gru2 = nn.GRU(input_size=self.scale*2, hidden_size=self.scale, num_layers=1, batch_first=True, bidirectional=True)

        self.l2 = nn.Linear(self.scale*2, PreNum*PreLen)
